...
export const CARDS = [
  ...
  },
  {
    ...
    scene: {
      ...
      // 누끼를 **카드 그림에서 직접 오려냈다.** 그래서 이 값은 근사치가 아니라
      // 실측이다 — 카드 안에서 누끼가 차지하는 자리 그대로라, 틀이 녹는 동안
      // 캐릭터가 한 픽셀도 안 움직인다 (배추와 같은 방식).
      //
      // 처음엔 배경 장면 렌더에서 딴 고해상도 누끼(820x888)를 썼는데, 그건 카드에
      // 인쇄된 그림과 **다른 렌더**라 자리를 맞춰도 들어갈 때 캐릭터가 튀었다.
      // 그림창은 틀의 알파에서 읽는다 (카드 좌표 x 53~769, y 161~878).
      // 누끼는 GrabCut 으로 딴다 — 임계값만으로는 무지개 홀로 배경에서 깨끗이 안 떨어진다.
      // 확실한 주인공(고구마 살·잎·아주 어두운 곳)과 확실한 배경(창 밖 + 아주 밝은 홀로)을
      // 씨앗으로 주고 6회 돌린 뒤, 캐릭터에 없는 색(G 가 R 보다 훨씬 높은 네온 초록,
      // 청록·파랑)을 빼고 제일 큰 덩어리만 남긴다.
      fit: { x: 6.13, y: 14.67, w: 86.52, h: 64.18 },
      motes: 52,
