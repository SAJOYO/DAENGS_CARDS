...
export const CARDS = [
  ...
  },
  {
    ...
    h: 1125,
    rarity: "mosaic",
    // **이머시브가 아니다.** 누끼가 카드 밖으로 튀어나오는 팝아웃만 한다.
    // 그래서 scene 이 아니라 pop 이다 — scene 을 주면 isImmersive 가 참이 되어
    // 원치 않게 이머시브 카드가 된다 (immersive.mjs 의 isImmersive 는 scene 을 본다).
    //
    // fit 은 템플릿 매칭으로 찾았다. 받은 누끼가 카드 대비 세로로 3.2% 눌려 있어서
    // (가로 0.620 · 세로 0.640) 이미지를 1279x1176 → 1279x1214 로 늘려 비율을
    // 1.0876 → 1.0535 로 맞춘 뒤 다시 쟀다. 상추(11% 눌림)와 같은 문제인데 정도가
    // 약했다 — 등방 매칭 점수가 0.71 로 애매하면 이방부터 의심하는 게 맞다.
    pop: {
      subject: "art/tomato-subject.webp",
      fit: { x: 2.21, y: 15.64, w: 97.18, h: 66.93 },
    },
    accent: "#cc351a",
    accent2: "#f29483",
    frame: "leaf",
